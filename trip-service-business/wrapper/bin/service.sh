APP_NAME=cmsms-service-business
APP_LONG_NAME=cmsms-service-business
...
# Build the lock file clause.  Only create a lock file if the lock directory exists on this optPlatform.
LOCKPROP=
if [ -d $LOCKDIR ]
then
    if [ -w $LOCKDIR ]
...
checkUser() {
    ...
    then
        ...
        then
            ...
            eval echo `gettext '        If user \"$RUN_AS_USER\" has a no shell, you can specify one using SU_OPTS if your optPlatform support it.'`
            eval echo `gettext '        For example, at the top of this script you can set: SU_OPTS=\"-s /bin/bash\".'`
            eval echo `gettext '        Another workaround would be to use a OS service management tool if available on your optPlatform.'`
