package com.yc.trip.api.business.service.visa;

import java.util.List;


import org.go.api.core.integration.AbstractDubboNativeService;
import org.go.api.core.util.BeanMapping;
import org.go.framework.core.exception.PendingException;
import org.go.framework.util.common.StringUtil;
import org.springframework.beans.factory.annotation.Autowired;

import com.alibaba.dubbo.config.annotation.Service;
import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;

import com.yc.trip.api.core.constants.ResCode;
import com.yc.trip.api.business.dao.visa.VisaDao;
import com.yc.trip.api.business.dto.visa.Visa;
import com.yc.trip.api.business.query.visa.VisaQuery;
import com.yc.trip.api.business.facade.visa.VisaFacade;

/**
 * 签证信息相关接口实现
 * 
 * @author My-Toolkits
 * @since 2019-01-06 18:03
 */
@Service(version = "1.0.0")
public class VisaFacadeImpl extends AbstractDubboNativeService implements VisaFacade {

    @Autowired
    private VisaDao visaDao;

    /**
     * 新增签证信息
     * 
     * @throws PendingException
     */
    @Override
    public Visa add(Visa visa) throws PendingException {
        try {
            // 新增时对各字段进行非空校验
            visa.validateInsertFields();
            // 调数据库接口进行新增操作
            visaDao.add(visa);
            // 将新增后回返回（包含自增主键值）
            return visa;
        } catch (Exception ex) {
            // 对异常进行处理
            throw transferException(ex, ResCode.visaDBError, "签证信息新增失败");
        }
    }

    /**
     * 修改签证信息
     * 
     * @throws PendingException
     */
    @Override
    public void update(Visa visa) throws PendingException {
        try {
            // 更新或删除操作时，不能所有参数都为空
            if (visa.isAllFiledsNull()) {
                ResCode.visaDBParamInvalid.throwException("签证信息更新时参数未通过校验");
            }
            // 调数据库接口进行更新操作
            visaDao.update(visa);
        } catch (Exception ex) {
            // 对异常进行处理
            throw transferException(ex, ResCode.visaDBError, "签证信息更新失败");
        }
    }

    /**
     * 查询签证信息
     * 
     * @throws PendingException
     */
    @Override
    public Visa get(VisaQuery visaQuery) throws PendingException {
        try {
            // 调数据库接口查询对象
            return visaDao.get(visaQuery);
        } catch (Exception ex) {
            // 对异常进行处理
            throw transferException(ex, ResCode.visaDBError, "签证信息查询失败");
        }
    }
    
    /**
     * 查询签证信息信息（若不存在，则抛出异常）
     * 
     * @throws PendingException
     */
    @Override
    public Visa mustGet(VisaQuery visaQuery) throws PendingException {
        // 查询单位信息
        Visa result = get(visaQuery);
        // 若不存在，则抛出异常
        if(result == null){
            ResCode.visaDBGetNull.throwException("未查询到签证信息");
        }
        return result;
    }

    /**
     * 查询签证信息列表
     * 
     * @throws PendingException
     */
    @Override
    public List<Visa> queryList(VisaQuery visaQuery) throws PendingException {
        try {
            // 在上下文中设置排序信息
            if (StringUtil.isNotBlank(visaQuery.getOrderby())) {
                PageHelper.orderBy(visaQuery.getOrderby());
            }
            // 调数据库接口查询列表
            return visaDao.queryList(visaQuery);
        } catch (Exception ex) {
            // 对异常进行处理
            throw transferException(ex, ResCode.visaDBError, "签证信息列表查询失败");
        }
    }

    /**
     * 查询签证信息列表 ,分页查询
     * 
     * @throws PendingException
     */
    @Override
    public PageInfo<Visa> queryPage(VisaQuery visaQuery) throws PendingException {
        try {
            // 对请求参数进行校验
            if (visaQuery.getPageNo() <= 0 || visaQuery.getPageSize() <= 0) {
                ResCode.visaDBParamInvalid.throwException("分页参数设置有误");
            }
            // 在上下文中设置分页信息
            PageHelper.startPage(visaQuery.getPageNo(), visaQuery.getPageSize());
            // 在上下文中设置排序信息
            if (StringUtil.isNotBlank(visaQuery.getOrderby())) {
                PageHelper.orderBy(visaQuery.getOrderby());
            }
            // 调数据库接口查询列表
            List<Visa> resultList = visaDao.queryList(visaQuery);
            // 返回分页结果
            return new PageInfo<>(resultList);
        } catch (Exception ex) {
            // 对异常进行处理
            throw transferException(ex, ResCode.visaDBError, "签证信息分页查询失败");
        }
    }

}
